...
def score_hw2(base_url):
    def validate_get(obj):
        print
        "validating GET: " + obj.url
        score = 0
        try:
            if (obj.status_code == 200):
                print
                "GL.1 Status code 200 OK"
                score = 10
            data = json.loads(obj.text)
            if len(data) > 1000:
                print
                "GL.2 List of DATES OK"
                score = score + 10
            if data[0]['DATE'] >= '20130101':
                print
                "GL.3 Checking Date 20130101 OK"
                score = score + 10
        except Exception, e:
            print
            "Error:", e
    ...
    def validate_get_obj(obj, date):
        ...
        try:
            ...
            if ((len(data) == 3) and ('TMAX' in data) and ('TMIN' in data)):
                print
                "L.1 Fields Check TMAX, TMIN and dict Size = 1 OK"
                score = score + 10
            if data['DATE'] == date:
                print
                "L.2 Checking for DATE " + date + "  OK"
                score = score + 10
    ...
    def validate_get_tmax(obj, tmax):
        ...
        try:
            if (obj.status_code == 200):
                print
                "G.1 Status Code 200 OK"
                score = 10
            data = json.loads(obj.text)

            if ((len(data) == 3) and ('TMAX' in data) and ('TMIN' in data)):
                print
                "G.2 Fields Check TMAX, TMIN and dict Size = 1 OK"
                score = score + 10

            if int(data['TMAX']) == tmax:
                print
                "G.3 Checking for TMAX " + str(tmax) + "  OK"
                score = score + 10
    ...
    def validate_post(obj, date):
        print
        "validating POST: " + obj.url
        score = 0
        try:
            if (obj.status_code == 201):
                print
                "P.1 Status Code 201 OK"
                score = 10
            data = json.loads(obj.text)

            if data['DATE'] == date:
                print
                "P.2 Checking for DATE " + date + "  OK"
                score = score + 10
    ...
    def validate_del(obj):
        print
        "validating DELETE: " + obj.url
        score = 0
        try:
            if (obj.status_code == 204 or obj.status_code == 200):
                print
                "D.1 Status Code 204/200 OK"
                score = 10
    ...
    def validate_forcasts(obj, date):
        ...
        try:
            if (obj.status_code == 200):
                print
                "F.1 Status code 200 OK"
                score = 10
            data = json.loads(obj.text)
            if len(data) == 7:
                print
                "F.2 List of DATES OK"
                score = score + 5
            if data[0]['DATE'] == date:
                print
                "F.3 Checking Date " + date + " OK"
                score = score + 5

            if (float(data[6]['TMAX']) > -30.0) and (float(data[6]['TMAX']) < 150.0):
                print
                "F.4 Checking TMAX in range -30 and 100  " + str(data[6]['TMAX']) + " for date " + date + " OK"
                score = score + 5
    ...
    print
    "Total Score  without BONUS Evaluation:", 100 * total_score / 130.0
    ...
    print
    "Total Score after BONUS Evaluation:", total_score
